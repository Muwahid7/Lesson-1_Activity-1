...
class RestaurantOrderApp:
    def __init__(self, root):
        ...
        bg_image = Image.open("Burger.jpg").resize((950, 650))
        self.bg_photo = ImageTk.PhotoImage(bg_image)
        bg_label = tk.Label(root, image=self.bg_photo)
        bg_label.place(x=0, y=0, relwidth=1, relheight=1)
        fg_image = Image.open("Pani puri.jpeg").resize((100, 100))
        ...
        self.items = {
"Pizza": {"price": 250, "var": tk.IntVar(), "qty": tk.StringVar(value="1")},"Pasta": {"price": 180, "var": tk.IntVar(), "qty": tk.StringVar(value="1")},"Salad": {"price": 120, "var": tk.IntVar(), "qty": tk.StringVar(value="1")},"Soup": {"price": 100, "var": tk.IntVar(), "qty": tk.StringVar(value="1")},"Juice": {"price": 90, "var": tk.IntVar(), "qty": tk.StringVar(value="1")}}
        y = 250
        for item, data in self.items.items():
            ...
            y += 30
            tk.Button(root, text="Submit Order", command=self.submit_order,font=('Arial', 12, 'bold'), bg="green", fg="white").place(x=70, y=430)
            tk.Button(root, text="Save & Print Bill", command=self.save_and_print,font=('Arial', 12, 'bold'), bg="blue", fg="white").place(x=220, y=430)
    ...
    def submit_order(self):
        ...
        for item, data in self.items.items():
            ...
            if not items_ordered:
                messagebox.showwarning("No Items", "Please select at least one ite")
                return 
